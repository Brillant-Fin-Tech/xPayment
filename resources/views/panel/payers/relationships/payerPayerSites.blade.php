                ...
                <tbody>
                    @foreach($payerSites as $key => $payerSite)
                        <tr data-entry-id="{{ $payerSite->id }}">
                            ...
                            <td>
                                @can('payer_site_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('panel.payer-sites.show', $payerSite->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('payer_site_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('panel.payer-sites.edit', $payerSite->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('payer_site_delete')
                                    <form action="{{ route('panel.payer-sites.destroy', $payerSite->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
