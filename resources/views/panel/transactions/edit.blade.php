...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("panel.transactions.update", [$transaction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('commission_rate') ? 'is-invalid' : '' }}" type="number" name="commission_rate" id="commission_rate" value="{{ old('commission_rate', $transaction->commission_rate) }}" step="0.01" max="100">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('commission') ? 'is-invalid' : '' }}" type="number" name="commission" id="commission" value="{{ old('commission', $transaction->commission) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_net') ? 'is-invalid' : '' }}" type="number" name="amount_net" id="amount_net" value="{{ old('amount_net', $transaction->amount_net) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $transaction->date) }}" required>
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payer') ? 'is-invalid' : '' }}" name="payer_id" id="payer_id" required>
                    @foreach($payers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('payer_id') ? old('payer_id') : $transaction->payer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('payer'))
                    <div class="invalid-feedback">
