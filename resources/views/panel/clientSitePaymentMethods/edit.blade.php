...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.clientSitePaymentMethod.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.client-site-payment-methods.update", [$clientSitePaymentMethod->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('client_payment_method') ? 'is-invalid' : '' }}" name="client_payment_method_id" id="client_payment_method_id">
                    @foreach($client_payment_methods as $id => $entry)
                        <option value="{{ $id }}" {{ (old('client_payment_method_id') ? old('client_payment_method_id') : $clientSitePaymentMethod->client_payment_method->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('client_site') ? 'is-invalid' : '' }}" name="client_site_id" id="client_site_id">
                    @foreach($client_sites as $id => $entry)
                        <option value="{{ $id }}" {{ (old('client_site_id') ? old('client_site_id') : $clientSitePaymentMethod->client_site->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
