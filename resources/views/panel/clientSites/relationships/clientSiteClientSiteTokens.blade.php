                        ...
                        <tr data-entry-id="{{ $clientSiteToken->id }}">
                            ...
                            <td>
                                @can('client_site_token_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('panel.client-site-tokens.show', $clientSiteToken->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('client_site_token_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('panel.client-site-tokens.edit', $clientSiteToken->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('client_site_token_delete')
                                    <form action="{{ route('panel.client-site-tokens.destroy', $clientSiteToken->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
