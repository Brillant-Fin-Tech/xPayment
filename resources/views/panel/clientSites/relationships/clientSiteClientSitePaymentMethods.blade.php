@can('client_site_payment_method_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('panel.client-site-payment-methods.create') }}">
                        ...
                        <tr data-entry-id="{{ $clientSitePaymentMethod->id }}">
                            ...
                            <td>
                                @can('client_site_payment_method_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('panel.client-site-payment-methods.show', $clientSitePaymentMethod->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('client_site_payment_method_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('panel.client-site-payment-methods.edit', $clientSitePaymentMethod->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('client_site_payment_method_delete')
                                    <form action="{{ route('panel.client-site-payment-methods.destroy', $clientSitePaymentMethod->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('client_site_payment_method_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('panel.client-site-payment-methods.massDestroy') }}",
